...
目录里 12 个示例的 `_note` 字段（一行一个）如下。**这个字段会显示在界面状态栏**：
...
| `12_799325.json` | **25x7 长条问号题**（64 冰、2 条箭头，唯一解）：已知数字 5 个 + 2 个「?」冰格，`5 + 2x2 = 9 = 最大数字` ⇒ 两个「?」**必须**各被十字交叉穿越两次（与 db039 同一条计数推论，见 [`../docs/算法说明.md`](../docs/算法说明.md) §3.11）。本目录里求解代价最重的一道（2.8 万分支节点，约 13 s） |
...
反过来（`icelom-v1` JSON → Penpa+ 求解链接）用 `python -X utf8 tools/json2penpa.py example/12_799325.json`：
冰格底色、冰道描边、数字与 "?"、箭头方向、IN/OUT 文字都按 Penpa+ 的编码落位，
逐元素判据见 [`../tests/test_penpa_export.py`](../tests/test_penpa_export.py)（该测试拿上面这条官方链接当外部基准）。
...
- 全部由 `python tools/make_examples.py` 生成：`01` 是原 9x9 夹具、`02` 是 8x8 博客题，
  ...
  来源 URL 见上一节），
  `12` 是**25x7 长条问号题**（本仓库作者自制，用本项目求解器反复验证到唯一解；题面与 2 条箭头同样
  **内联在脚本里**，没有可解码的来源 URL）。
- 脚本会**整体重写**这 12 个文件（含 `_note`），所以不要手工改这些 JSON ——
  要改就改 `tools/make_examples.py`（这也意味着 `_note` 的文字只在生成时写一次，
  与本题面后来的求解结论可能不再同步）。
  （示例 JSON 在仓库里是 CRLF；生成后应与仓库里的文件**逐字节一致**，即跑完
  `python tools/make_examples.py` 之后 `git status` 里 `example/` 不应有任何改动。）
...
（`12_799325.json` 按要求排到最后，因此不影响界面启动时默认加载的 `01`。）
