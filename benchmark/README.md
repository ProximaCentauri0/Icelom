...
**`symmetry_retry` 已是死开关**：求解器只有**一颗搜索树 + 一个固定分支顺序**
（规则传播 + 单边试连接推到不动点，剩下的按行主序固定顺序枚举），不再有换扫描序/换朝向重试，
...
- **db039**（13×13、23 冰、4 个 "?"、数字 1,3,…,17）的解已求得：
  ...
  经官方 pzprjs 引擎判定 **PASS**（`tools/official_check.py db039 analysis/db039_solution.json`）。
  它与"把作者提示作为强制边"求得的解**逐格完全相同**（两条独立路径互证）；
  解与渲染图见 [`../analysis/db039_solution.json`](../analysis/db039_solution.json) /
  [`../analysis/db039_solution.png`](../analysis/db039_solution.png)。
