...
def write_report(results, args, total_wall):
    ...
    A(f"- 求解器: `icelom_solver.exe` (g++ -O2; **规则传播 + 单边试连接**推理到不动点,"
      f" 剩下的分支按**行主序固定顺序**枚举; 数字按位次规则判定, 支持跳号与 \"?\" 格)")
    ...
    A(f"| **当前（规则传播 + 单边试连接, 固定顺序枚举）** | {len(results)} "
    ...
    A("> **当前版本把 `tools/deduce.py` 的确定性推理搬进了求解器核心**：每条规则（格内配置、"
      ...
      "一路推到不动点，剩下的才交给搜索；分支顺序是**行主序固定顺序**（不做 MRV 选格、"
      "不换扫描序/换朝向重试）。")
    A(">")
    ...
    A("> 该解为 171 格线路，编号位次恰为 `1,?(2),3,?(4),…,?(16),17`，独立验证器通过，"
      ...
      "与\"把作者提示作为强制边\"求得的解**逐格完全相同**。产物见 `analysis/db039_solution.json`、"
      "`analysis/db039_solution.png`、`analysis/db039_report.txt`。")
    A("")
