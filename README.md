...
后端默认先单线程求解；1 秒后当前模式仍未完成，自动升到 8 线程（GUI 同样生效）。
输入 JSON 的 `limits.threads` 默认为 `0`，表示上述自适应策略；`1` 固定单线程，`2..64` 立即使用指定线程数。详见
[协议](docs/求解器协议.md)和[性能优化报告](benchmark/OPTIMIZATION.md)。
求解核心：**规则级约束传播 + 单边试连接（推到不动点）+ 两级打分选格枚举** ——
...
`icelom_render.py` 另提供 Nikoli 纸面风格的离屏渲染（可导出 PNG）。*
...
> 压缩包里只有"运行必需"的东西：界面 + 静态 exe + 图标 + 12 道示例题面 + URL 解码器 + 包内 README。
...
3. **冰格（蓝格）**：不能转弯（进入后必须直行穿出），**允许垂直交叉**（每格横、竖各最多一次，
   ...
   **IN 与 OUT 也可以同处一个冰格**（只能是两条**互相垂直**的外边框，例如同一个角冰格）：
   该格被横竖各穿一次 —— 一条外边框入界、另一条出界，与"冰格允许垂直交叉"是同一条规则
   （回归：`tests/run_tests.py` T11.6；官方引擎判定见 `tools/official_check.js`）。
...
| `icelom_gui.py` | Python tkinter 图形界面（编辑器 + 求解 + puzz.link 式多段画线 + 规则引擎「检查解」/「对照解」+ 解存档 + 导出 Penpa+ 链接） |
...
| `example/` | GUI 示例题（下拉框数据源，12 道；见 `example/README.md`） |
| `benchmark/` | 65 道真实题面的性能基准 + 计时/验证/报告工具（见 `benchmark/README.md`、`benchmark/REPORT.md`） |
| `tools/` | URL 导入、官方判定、长跑求解、渲染、**题面导出 Penpa+ 链接（`json2penpa.py`）**、**规则推导器（`deduce.py` / `deduce_report.py`）**、示例生成、图标生成、`taskset.ps1`、vendored `pzprjs/` |
| `tests/` | 回归测试、独立验证器、独立求解模型、官方引擎差分、GUI 测试（见 `docs/测试与验证.md`） |

> 分析/审计类产物（推导报告、审计报告、渲染图等）不再落盘保留：需要时按
> [`docs/测试与验证.md`](docs/测试与验证.md) 的命令重生成，产物目录会被脚本自动创建。
...
两边对得上，任务栏就显示 IceLom 的图标。改了 `assets/icelom.ico` 后要**两步**：重跑一次脚本
**并清掉 Explorer 图标缓存** —— 原地换 `.ico` 内容时 Windows 往往不失效缓存，任务栏/桌面会继续
显示旧图（本项目实测过，见 `docs/历史与回归基线.md` §15.1）。

> 图标是**统一缩放**的：`.ico` 各帧（16~256 px）与各尺寸 PNG 全部由同一张 512 px
> 完整题面图**逐尺寸 LANCZOS 缩小**而来，任何尺寸下看到的都是同一幅画。
> （早期曾给 ≤64 px 单独画过一版"简化版"，两套画法放一起观感不搭，已改回统一缩小版；
> 逐尺寸预缩这一点保留 —— 让 Pillow 自己缩大图仍是低质量重采样。）
...
- **白/冰**：点击涂一个格，**按住拖动可连续涂一片**（一笔只占一条撤销记录）；
  快速甩动时会把两次鼠标事件之间跨过的格子**逐格补齐**（不会漏格），整次事件只重绘一次。
- **数字**：工具条上有一个**数字选择器**（`数字 [ 4 ▲▼ ]`），它就是"数字工具下一个要放的数"：
  ...
  - **「新建」后选择器回到 1**：空白盘面从 1 开始编号，所以新建（以及"调整大小"之后）
    选择器不会还停在上一次用到的数字上；重置后连点即得 1,2,3,…
...
- **IN / OUT**：**点击格子**放置（再点同格取消自己那一个端点）：
  - **内部格**：线路以该格格心为起/终点，格内显示带衬底的 IN/OUT 字样，无箭头；
  - **边界格**：显示入/出界黑色箭头（方向自动生成）——箭头长 1 个格子、**关于外框线对称**
    （中心正好在边界格那条边的中点上，IN 与 OUT 等长），标签画在箭头**正后方**、沿箭头轴居中；
  - **角上的冰格**：入/出界方向不唯一，点击后进入"指定方向"流程——再点击相邻格
    （IN=线路去向、OUT=线路来向）完成设置，Esc 取消；
  - **同一个角冰格上放 IN 和 OUT**（两条外边框互相垂直）：先放的那个走上面的流程；
    再点同一格放另一个时方向已经唯一（只剩另一条边），**直接落在剩下那条边上、两者共存**
    —— 该格被横竖各穿一次（见规则 3）；
  - 其余"同格"组合仍是**顶掉**同格的另一端点：白格至多经过一次；1 行 / 1 列的盘面上
    两条**同轴**外边框只能构成退化的一次直穿，界面不为这类组合做自动放置
    （手写 JSON 仍可用：求解器与规则引擎照常按规则判定）。
  - **1 行 / 1 列的盘面**上格子可能只有两条**同轴**外边框：此时不进"指定方向"流程
    （点相邻格给不出同轴方向），而是**点靠近哪条外边框就放在哪条边上**，点格子中间取靠前那条。
- **线段 / 箭头 / 墙**：**点击起点格再点击相邻格（或直接拖动）**即生成，
  **箭头方向 = 行进方向**；**线段与箭头长 1 个格子**（从一个格心到另一个格心，中心 = 所穿过
  边的中点），墙是平行于该边的红粗线；重复创建同一线段/箭头/墙取消；每次创建完成后**不保留任何
  选择状态**——想画反向箭头必须从头重新选起点，想删箭头就从头再创建一次相同的箭头。
  起点格选定后按 **Esc** 取消。
- **橡皮**：清除格子内容或边标记（**右键**任何时候都是橡皮；做题模式右键=清除线路）。
- **Ctrl+Z** 或"撤回"按钮撤销；求解中撤回会**先打断求解**；行列数改后点"调整大小"；
  谜题可 保存/打开（JSON）：**保存**（**Ctrl+S**）写回已打开的那份文件（没打开过就先问路径），
  **另存为…**（**Ctrl+Shift+S**）每次都问路径，之后「保存」写的是这份新文件。
...
- **导出 Penpa+ 链接**：界面上点 **「导出 Penpa+ 链接」**（工具条按钮，或菜单
  **文件 → 导出 Penpa+ 链接…**；命令行等价物是 `python -X utf8 tools/json2penpa.py <谜题.json>`）
  会把题面 JSON 变成 Penpa+（penpa-edit）**求解链接**——棋盘比题面大一圈留白、冰格连成的
  "冰道"与题面外框用淡蓝粗线描出、数字与 "?" 原样落格、箭头按方向画在对应边上、IN/OUT
  写在框外并带同向箭头，规则文本写进链接自带的规则框。链接里的题面元素与 Penpa+ 自己
  产出的编码逐元素对齐（见 `tests/test_penpa_export.py`）。
  - **配置窗里选答案**：要不要**对照答案**（链接带 `a` 参数，页头显示 "Solver Mode (Answer
    Checking Enabled)"，**画完点 Check 就能自动判定**），以及答案用哪一份——
    **本题当前的解** / **本题在解存档里的解** / **手动绘制核验解** / **不带答案**。
    默认取能用的第一个（当前的解 > 存档解 > 不带答案）。
  - **手动绘制核验解**：选它之后盘面**回到空题面**（解与既有线路都先清掉），
    在盘面上**自己画一条线**——画什么、画成什么样都行，**那条线原样当对照答案，不做任何验证**；
    画完点 **「完成手动绘制」**就出链接（点「取消」原样还原）。
    用途是"我认为答案长这样，拿去页面上核验"：链接里带的就是手画的这条线。
    在 IN/OUT 处画出框外的那一步可以不画，链接会自己补上进出框那两段。
  - **怎么判过**：带答案的链接开的是**精确判定**（`sol_loopline_exact`），页面比的是
    "画出来的线"与链接里的答案**是否逐字节相同**，所以**要用答案那个颜色画**——
    本工具统一把答案记成 **9 号（淡蓝，页面上那排颜色的第 6 个）**，与题面外框同色；
    答案来源是本题的解时，链接里的答案就是求解器给的那条线路（进出框两段已补好）。
  - 弹出窗里可以直接**复制**（打开即已复制到剪贴板）/ **存为文件** / **在浏览器打开**。
  - **连带写入存档**：导出"本题的解 / 本题的存档解"时会把链接存进解存档那条记录
    （`links.puzzle` = 题面链接、`links.answer` = 带本题答案的链接），所以"这道题的 Penpa+
    链接"随时都能从存档里取回，不必重新导出；**手动绘制的那条线不写存档**
    （一次性草稿，免得下次被当成这道题的解）。
  - 命令行工具 `tools/json2penpa.py` 也读同一份存档：答案来源优先级
    `--solution` > **解存档** > `--solve`（`--no-archive` 关掉，`--archive` 换存档路径）；
    判定口径与界面完全一致（没有"只比形状"的开关）。
### 求解

- 勾选/取消"**必须经过所有白格**"（默认勾选，即官方规则）。
- **求解模式**（**默认「判断唯一性」**）：
  - **判断唯一性**（默认）：穷尽搜索找一个解, 报告"解是唯一的"或"解不唯一(至少 2 个)"
    并列出两个解; **推理预算内没证完穷尽时, 求解器保守按"不唯一"报数**（此时并没有真的
    找到第 2 个解）, 界面会如实提示"未能证完穷尽"并建议调大「推理预算」重试;
  - **只求解**：找到第一个解立即返回（最快）;
  - **求出所有解**：枚举全部解, 用"解数量上限"控制（默认 20, 仅此模式可设）。
- **推理预算（秒）**：本次求解的时限, 默认 120 秒, 可直接输入（1..86400, 非法输入回退
  默认值）; 界面记住最近一次用的值（`%APPDATA%/IceLom/gui.json`）。
- **解存档**：求解器**证明解唯一**时（判断唯一性/求出所有解 恰得 1 解且未被截断;
  "只求解"不证明唯一, 不入档）自动把解写入 `%APPDATA%/IceLom/solutions.json`。
  再打开同一道题（按**题面内容**识别, 改名/挪目录/URL 导入都算同一题）时自动载入存档解,
  可直接「显示解」; 此时点「求解」会先询问是否重新求解。
  同一条记录里还会留下这道题的 **Penpa+ 链接**（题面链接 + 带本题答案的链接），
  格式见下文「解存档」小节。
...
界面求解出**唯一解**时自动写入；文件由界面与 `tools/json2penpa.py` 共读：

```json
{
  "version": 1,
  "solutions": {
    "<题面内容指纹 puzzle_key(SHA-256)>": {
      "path": [[0,4], [0,5], "…"],                    // 解 = 逐格序列（含冰格交叉的重复经过）
      "saved_at": "2026-09-18 22:33:31",
      "source": "icelom_gui", "file": "打开时那份题面.json",
      "w": 9, "h": 9,
      "puzzle": { "format": "icelom-v1", "…": "题面本身（记录自解释，见下）" },
      "links": { "puzzle": "…不带答案的 Penpa+ 链接",
                 "answer": "…带本题答案的 Penpa+ 链接" }
    }
  }
}
```

- **键 = 题面内容指纹**，与文件路径无关：改名/挪目录/URL 导入都算同一题（`puzzle_key` 只取
  `w/h/cells/numbers/edges/in/out` 这些语义字段，与键序、`format`/`options`/`limits` 无关）。
- **`puzzle` 让记录自解释**：有题面才能按它把两种链接现算出来，只留尺寸与路径是画不出来的。
- **`links` 是缓存**：读取 `links.puzzle` / `links.answer` 时按存档里的题面与解**现算**，
  所以旧格式/旧画法的链接不会被喂出去。**手动绘制的那条核验线不进存档**（见「导出」节）。
- 存档读写一律"先写临时文件再 `os.replace`"，且**任何错误都不抛异常**（坏存档 = 没有存档）。
...
python -X utf8 tools\deduce_report.py benchmark\puzzles\db039_13x13_0vanillaice0.json --png db039_deduce.png   # "光靠规则能推到哪一步"的推导报告（解缺省时现场求解）
...
- 基准 65 题（`python benchmark/run_benchmark.py` → [`benchmark/REPORT.md`](benchmark/REPORT.md)）：
  **65 题全部唯一解、0 超时**，65 题 × 3 遍总墙钟 **约 5 s**（每遍约 20 ms 是进程启动开销；
  ...
  另有 24 题 1~66 个分支节点，最慢的 db048 也只有 2092 个节点 / 约 120 ms；
- **db039 不再是难例**（13×13、23 冰、4 个 "?"、数字 1,3,…,17）：旧引擎在题面朝向下 300 s
  ...
  与"把作者提示作为强制边"求得的解**逐格完全相同**。解与报告不再落盘保留，
  `python tools\official_check.py db039` 可现场重解并判定；
- **同一个推理层在 `tools/deduce.py` 里也有独立实现**（`tools/deduce_report.py` 出报告）：
  ...
  **0 题推错、0 题误禁、0 题假矛盾，33 题被推理直接推完整条线路**且逐题通过独立验证。
  ...
  先 `python tools\deduce.py <题.json> --out <状态.json>`，再 `python icelom_gui.py --state <状态.json>`；
...
4. **两级打分选格枚举**（推理推不动时才用）：在还能分支的"必有格"里选
   **合法配置数最少**的格，平手看**未定边邻居的平均合法配置数**（少者优先），
   仍平手按行主序 —— 打分全部来自一次遍历可得的已知量，不做任何试算；
   分支 = 决定该格的最终连接集合。不换扫描序、不换盘面朝向。
...
- **题面**（`benchmark/puzzles/`、`example/`、`everyday_puzzle/`）：来自作者**公开发布**的题目，经
