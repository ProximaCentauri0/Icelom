...
## 2. 题面数据（`benchmark/puzzles/`、`example/`、`everyday_puzzle/`、`assets/icon_puzzle.json`）
...
- 来源：**各作者公开发布**的 アイスローム（IceLom）题目，经 [puzz.link/db](https://puzz.link/db/)
  ...
  （作者 Hexi），来源 URL 记在 `example/README.md`。`everyday_puzzle/` 是每日题中
  保留下来的两道高难度样本（20260915 的原始 Penpa+ 链接见同目录 `.txt`），仅作
  补充测量输入，见 `everyday_puzzle/README.md`。
