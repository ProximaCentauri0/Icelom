...
（它们在 GitHub 仓库里，包内 README 给出仓库地址）。产出（release/ 可整体拷走或打包分发）:
    ...
    ├─ example/*.json             GUI 示例下拉框数据源（12 道题面）
    └─ tools/
...
FILES = [
    ...
    ("tools/json2penpa.py", "tools"),      # 界面「导出 Penpa+ 链接」按路径 import 它
...
- **后端**：C++ 单文件求解器 `icelom_solver.exe`（静态链接，**单文件无 DLL 依赖**）；
  默认单线程起步，当前模式 1 秒仍未完成自动升到 8 线程（输入 `limits.threads` 可固定 1 或指定 2..64）
...
界面顶部：**示例**下拉框（内置 12 道题）、新建 / 打开 / 保存 / **另存为** / **导入 URL**；
**编辑**模式涂白/冰、放数字与 "?"、放 IN/OUT、画线段/箭头/墙；
**做题**模式按住拖动即可画线（可分段、可从任意格起笔、可在 IN/OUT 处画出界外），
「检查解」按规则实时判定，「显示解 / 隐藏解」对照答案，「对照解」只看当前线路是不是解的一部分。

**解存档**：求出唯一解时自动存进 `%%APPDATA%%/IceLom/solutions.json`（键 = 题面内容指纹，
与文件路径无关），再打开同一道题即可直接「显示解」；同一条记录里还留着这道题的
**Penpa+ 链接**。

**导出 Penpa+ 链接**：工具条按钮 / 菜单 **文件 → 导出 Penpa+ 链接…**。弹出的配置窗里选
要不要**对照答案**（页面画完点 Check 自动判定）、以及答案用**本题当前的解 / 本题的存档解 /
存档里另一道题的答案 / 不带答案**；链接可直接复制、存成文件或在浏览器打开。
导出**不需要联网**，也不需要 Node.js。
...
导出 Penpa+ 链接**不需要额外依赖**：`tools/json2penpa.py` 只用 Python 标准库。

## 命令行工具

```
node   tools/pzprurl2json.js "https://puzz.link/p?icelom/a/8/8/..../3/12"   # URL → 题面 JSON
python tools/json2penpa.py example/03_入门5x5.json --solve --out link.txt   # 题面 → Penpa+ 链接（带答案）
...
| `tools/json2penpa.py` | 题面 JSON → **Penpa+ 求解链接**（界面「导出 Penpa+ 链接」用的就是它） |
...
python tools/json2penpa.py example/03_入门5x5.json --stats   # Penpa+ 链接
...
def smoke_test(pkg):
    """在**打包后的裸目录**里验证：求解器能解题、URL 解码器能跑、Penpa+ 链接能生成。"""
    ...
    # 界面「导出 Penpa+ 链接」按路径 import tools/json2penpa.py —— 它必须在包里、
    # 而且能在**裸目录**里跑起来（只用标准库，所以不需要额外依赖）。
    penpa = os.path.join(pkg, "tools", "json2penpa.py")
    if not os.path.exists(penpa):
        return False, "包内缺少 tools/json2penpa.py（界面导出 Penpa+ 链接会不可用）"
    j = subprocess.run([sys.executable, "-X", "utf8", penpa, SMOKE_PUZZLE],
                       cwd=pkg, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    jout = j.stdout.decode("utf-8", "replace").strip()
    if j.returncode != 0 or "penpa-edit" not in jout:
        return False, "Penpa+ 链接冒烟失败: %s" % (
            j.stderr.decode("utf-8", "replace")[:200] or jout[:200])

    node = shutil.which("node")
    if not node:
        return True, "求解 OK；Penpa+ 链接 OK；未找到 node，跳过 URL 解码"
    ...
    return True, "求解 OK；Penpa+ 链接 OK；URL 解码 OK"
