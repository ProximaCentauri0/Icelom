...
class Board(object):
    ...
    def frame_conns(self, c):
        """该格的 IN/OUT **边框边**按轴统计: {轴: 条数}（0/1/2 条，空 = 不是端点格）。

        IN 与 OUT 可以落在同一格（同一个角冰格的两条互相垂直外边框，冰上垂直交叉），
        所以这里按轴分别计数 —— 与求解器 `frameConnsAxis` 是同一套口径：
        冰格每条轴的"边框边 + 格内边"总数都必须是 0 或 2。
        """
        n = {}
        p = self.xy(c)
        if p == self.IN and self.IN_side:
            ax = AXIS_OF[self.IN_side]
            n[ax] = n.get(ax, 0) + 1
        if p == self.OUT and self.OUT_side:
            ax = AXIS_OF[self.OUT_side]
            n[ax] = n.get(ax, 0) + 1
        return n
    ...
    def deg_bounds(self, c):
        ...
        口径与求解器的节点模型一致: **冰格每条轴的"边框边 + 格内边"总数必须是 0 或 2**;
        IN/OUT 的边框边按轴计数(`frame_conns`), 于是:

        * IN 格: 入框那条轴"边框 1 + 格内 1" ⇒ 1 条; 另一轴仍可 0 或 2 ⇒ {1, 3}
          (3 = 冰质 IN 之后再被垂直穿过一次);
        * OUT 格: 同理 {1, 3}("先垂直穿过再沿出框轴出框");
        * **同一格既是 IN 又是 OUT**(两条外边框互相垂直, 冰上垂直交叉): 两轴各
          "边框 1 + 格内 1" ⇒ 恰好 2 条;
        * 普通白格恰好 2 条; 普通冰格 0 / 2 / 4 条(两条轴各 0 或 2)。

        历史坑: 早先把边框边也算进配置的度数(`IN=(2,)` 靠"边框边 + 1 条格内边"凑),
        于是白质 OUT 格只剩下"边框边自己"这一种配置、冰质 OUT 格被垂直穿过时凑不出度数 ——
        db017 / db019 / db033 三道真题上直接报出**假矛盾**。第二个坑是"只认一条端点轴":
        同格 IN/OUT 会被算成"另一轴 0 或 2", 于是真解的那种 1 + 1 走法被判死、推导器报**假无解**。
        """
        k = self.kind(c)
        if k == "W":
            return (2,)
        if k == "I":
            return (0, 2, 4)
        fc = self.frame_conns(c)
        if not fc:
            # 内部 IN/OUT(无边框方向): 按普通格处理(本推导器只对边框 IN/OUT 做入框/出框约束)
            return (2,) if not self.is_ice(c) else (0, 2, 4)
        if not self.is_ice(c):
            # 白格在线路上时总连接数恰好 2 ⇒ 格内边 = 2 - 边框边
            return (2 - sum(fc.values()),)
        # 冰格: 逐轴取允许的格内边条数, 求和
        tot = {0}
        for ax in (0, 1):
            f = fc.get(ax, 0)
            per_ax = {2 - f} if f else {0, 2}
            tot = {a + b for a in tot for b in per_ax}
...
class Deduce(object):
    ...
    def cell_configs(self, c):
        ...
        * 端点轴: IN/OUT 的**边框边按轴计数**(`frame_conns`), 冰格每条轴要求
          "边框边 + 格内边"总数 ∈ {0, 2} ⇒ 该轴有 1 条边框边时正好 1 条格内边、
          有 2 条边框边(同一轴两侧都是 IN/OUT)时 0 条、没有边框边时 0 或 2 条;
          **同格 IN/OUT 的两条外边框互相垂直** ⇒ 两条轴各 1 条格内边(冰上垂直交叉);
        * **编号格必须被经过**: 已知数字的冰格只能被穿越一次(穿越次数 = 总连接数 / 2,
          总连接数 = 格内边 + 边框边); "?" 冰格按 `must_cross_twice` 至多/恰好两次。
        """
        b = self.b
        u = self.used_edges(c)
        ...
        def ok(sub):
            ...
            for ax in (0, 1):                              # 冰格: 每轴"边框边 + 格内边" 0 或 2
                if len(axs.get(ax, ())) + fc.get(ax, 0) not in (0, 2):
                    return False
            traversals = (d + f_tot) // 2                  # 穿越次数 = 总连接数 / 2
            if nv is not None and nv > 0:
                return traversals == 1                     # 已知数字的冰格: 恰好穿越一次
            if nv is not None and nv < 0:
                # "?" 冰格: 必须被经过(至少一次穿越); 被计数强制时必须十字交叉两次
                if traversals < 1:
                    return False
                if must2 and traversals != 2:
                    return False
            return True
    ...
    def _terminus(self, cur, prev):
        ...
        * **IN 与 OUT 同格**(两条外边框互相垂直): 沿 IN 轴进来是起点、沿 OUT 轴进来是终点 ——
          两条轴各判一次, 不能只看 IN 那一条。
        """
        b = self.b
        if prev is None:
        ...
        a_in = AXIS_OF[b.IN_side] if (is_in and b.IN_side) else None
        a_out = AXIS_OF[b.OUT_side] if (is_out and b.OUT_side) else None
        if a_in is not None and ax == a_in:
            return "IN"
        if a_out is not None and ax == a_out:
            return "OUT"
        return None
