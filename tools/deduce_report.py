...
用法:
    python -X utf8 tools/deduce_report.py <题.json> [--solution <解.json>]
                                         [--id db039] [--no-probe] [--png 图.png]
产物: stdout 报告 + 可选 PNG。解文件缺省时用本地求解器现场求首解当真解。
"""
import argparse
import json
import os
...
def main():
    ...
    ap.add_argument("--solution", default=None,
                    help="已知真解 JSON(用于核对); 缺省时用本地求解器现场求首解")
    ...
    if args.solution:
        ...
        p["limits"] = {"max_solutions": 1, "time_limit_ms": 60000}
        r = subprocess.run([os.path.join(ROOT, "icelom_solver.exe")],
                           input=json.dumps(p).encode(), stdout=subprocess.PIPE,
                           stderr=subprocess.DEVNULL)
        sols = [json.loads(l) for l in r.stdout.decode("utf-8", "replace").splitlines()
                if '"solution"' in l]
        if not sols:
            raise SystemExit("求解器没有出解, 无法做真解核对; 请先给 --solution 解.json")
        path = [tuple(q) for q in sols[0]["path"]]
